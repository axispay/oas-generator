...
var schemaV3 = fs.readFileSync(
  path.join(__dirname, "./schemas/openapi-3.0.yaml"),
  "utf8"
);
...
function generateServer(file, cmd) {
  if (!file) {
  ...
  } else if (!cmd.projectType) {
    console.log("You must select a project type!");
  } else if (cmd.projectType !== "bff" && cmd.projectType !== "microservice") {
    console.log(
      "Invalid project type, please select out of the following (bff, microservice)!"
    );
  } else if (semver.lt(process.version, "v8.0.0")) {
    console.log(
      "This program is not compatible with Node.js versions lower than v8.0.0 (current: " +
        process.version +
        ")"
    );
  } else {
    try {
      ...
      if (err == false) {
        logger.error(
          "oasDoc is not valid: " + JSON.stringify(validator.getLastErrors())
        );
        process.exit();
      }
      var projectName = "nodejs-server-generated";
      var projectType = cmd.projectType;
      var channel = cmd.channel || "portal";
      if (cmd.projectName) {
        // TODO: fix issues with program parameters...is the camel-case conversion the problem?? see commander npm docu
        projectName = cmd.projectName;
        if (!/^[a-zA-Z0-9-_]+$/.test(projectName)) {
          logger.error(
            "Name must only contain alphabetic characters, numbers and dashes."
          );
          process.exit();
        } else {
      ...
      if (cmd.json) {
        fs.writeFileSync(
          "./api/oas-doc.json",
          beautify(JSON.stringify(oasDoc), {
            indent_size: 2,
      ...
      fs.writeFileSync(
        ...
        beautify(JSON.stringify(package_raw), {
          indent_size: 2,
          space_in_empty_paren: true,
        })
      );

      /* create unique files: controllers, services, middlewares, responseHandler, exceptionHandler */
      ...
      if (!fs.existsSync("middlewares/rules")) {
        fs.mkdirSync("middlewares/rules");
      }

      if (!fs.existsSync("middlewares/response-handlers")) {
        fs.mkdirSync("middlewares/response-handlers");
        fs.copyFileSync(
          __dirname + "/auxiliary/response-handler.js",
          "middlewares/response-handlers/response-handler.js"
        );
      }

      if (!fs.existsSync("middlewares/exception-handlers")) {
        fs.mkdirSync("middlewares/exception-handlers");
        fs.copyFileSync(
          __dirname + "/auxiliary/exception-handler.js",
          "middlewares/exception-handlers/exception-handler.js"
        );
      }
      ...
      for (var oasPath in paths) {
        for (var method in paths[oasPath]) {
          if (paths[oasPath][method].operationId != undefined) {
            ...
            logger.debug(
              "Oas-doc does not have opearationId property for " +
                method.toUpperCase() +
                " - " +
                oasPath +
                " -> operationId name autogenerated: " +
                opId
            );
          }
          ...
          if (paths[oasPath][method]["x-router-controller"] != undefined) {
            ...
            paths[oasPath][method]["x-swagger-router-controller"] != undefined
          ) {
            controllerName =
              paths[oasPath][method]["x-swagger-router-controller"];
          } else {
            controllerName = generateName(oasPath, "controller");
            logger.debug(
              "Oas-doc does not have routing property for " +
                method.toUpperCase() +
                " - " +
                oasPath +
                " -> controller name autogenerated: " +
                controllerName
            );
          }
          ...
          if (!fs.existsSync("controllers/" + controllerName + ".js")) {
            ...
            var controllerVariable = generateName(controllerName, "service"); //sanitize variable name for controller's require
            var header =
              "'use strict' \n\nconst " +
              controllerVariable +
              " = require('../service/" +
              ...
              "-service');\n\n";
            fs.appendFileSync(
              process.cwd() + "/controllers/" + controllerName + ".js",
              header
            );
          }

          // if (!service_files.includes(controllerName + '-service')) {
          if (!fs.existsSync("service/" + controllerName + "-service.js")) {
            service_files.push(controllerName + "-service");
            var header = "'use strict' \n\n";
            fs.appendFileSync(
              process.cwd() + "/service/" + controllerName + "-service.js",
              header
            );
          }
          ...
          var controllerData = fs.readFileSync(
            process.cwd() + "/controllers/" + controllerName + ".js",
            "utf8"
          );
          if (controllerData.indexOf("module.exports." + opId) >= 0) {
            console.log("function already exists!!"); //Do Nothing
          } else {
            fs.appendFileSync(
              process.cwd() + "/controllers/" + controllerName + ".js",
              function_string
          ...
          var function_string_service =
            "module.exports." +
            opId +
            " = function " +
            opId +
            " (req, res, next) {\nres.send({message: 'This is the mockup controller for " +
            opId +
            "' });\n};\n\n";

          var serviceData = fs.readFileSync(
            process.cwd() + "/service/" + controllerName + "-service.js"
          );
          if (serviceData.indexOf("module.exports." + opId) >= 0) {
            console.log("function already exists in service!!"); //Do Nothing
          } else {
            fs.appendFileSync(
              process.cwd() + "/service/" + controllerName + "-service.js",
              function_string_service
            );
          }
        }
      }

      /* beautify controller files */
      for (var i = 0; i < controller_files.length; i++) {
        logger.debug("Beautify file " + controller_files[i]);
        var data = fs.readFileSync(
          process.cwd() + "/controllers/" + controller_files[i] + ".js",
          "utf8"
        );
        fs.writeFileSync(
          process.cwd() + "/controllers/" + controller_files[i] + ".js",
      ...
      /* beautify service files */
      for (var i = 0; i < service_files.length; i++) {
        logger.debug("Beautify file " + service_files[i]);
        var data = fs.readFileSync(
          process.cwd() + "/service/" + service_files[i] + ".js",
          "utf8"
        );
        fs.writeFileSync(
          process.cwd() + "/service/" + service_files[i] + ".js",
      ...
      process.chdir("..");
      if (cmd.generateZip) {
        //option -z used: generate zip and delete folder
        zipdir(
          "./" + projectName,
          {
